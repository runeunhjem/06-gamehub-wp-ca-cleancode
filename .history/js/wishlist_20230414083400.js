...
async function createGameElements() {
  // Wait for all games to be loaded
  await Promise.all(games.map((game) => game.load()));
  ...
  wishlist.forEach((e, index) => {
    ...
    // Create a new element for the game
    const gameElement = document.createElement("div");
    gameElement.classList.add("game");
    gameElement.innerHTML = `
      <img src="${game.image}" alt="${game.itemName}">
      <h2>${game.itemName}</h2>
      <p>${game.description}</p>
      <button>Add to cart</button>
    `;

    // Add the new element to the games container
